...
/**
 * Handle GET/POST /api/files
 * GET: ?commits=hash1,hash2,...
 * POST: body { commits: [hash1, hash2, ...] }
...
async function handleGetFiles(req, res, cwd) {
  try {
    ...
    if (req.method === 'POST') {
      ...
      commitHashes = parseCommitHashes(body.commits);
    } else {
      // Read from query string (backwards compatibility)
      const query = parseQueryString(req.url);
      commitHashes = parseCommitHashes(query.commits);
    }
...
/**
 * Handle GET/POST /api/diff
 * GET: ?file=path&commits=hash1,hash2,...
 * POST: body { file: path, commits: [hash1, hash2, ...] }
 * Returns consolidated diff for a file across multiple commits
 */
async function handleGetDiff(req, res, cwd) {
  try {
    let filePath, commitHashes;
    ...
    if (req.method === 'POST') {
      ...
      filePath = sanitizeFilePath(body.file);
      commitHashes = parseCommitHashes(body.commits);
    } else {
      // Read from query string (backwards compatibility)
      const query = parseQueryString(req.url);
      ...
      filePath = sanitizeFilePath(query.file);
      commitHashes = parseCommitHashes(query.commits);
    }
