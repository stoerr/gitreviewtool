...
/**
 * Parse commit hashes from comma-separated string or array
 * @param {string|string[]} commitsParam - Comma-separated commit hashes or array
...
function parseCommitHashes(commitsParam) {
  ...
  // Handle array input (from POST body)
  if (Array.isArray(commitsParam)) {
    return commitsParam.map(sanitizeCommitHash);
  }

  // Handle string input (from query parameter)
...
function readJSONBody(req) {
  return new Promise((resolve, reject) => {
    ...
    req.on('data', chunk => {
      body += chunk.toString();
      // Prevent too large payloads (10MB limit)
